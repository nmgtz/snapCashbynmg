 const scriptURL = 'https://script.google.com/macros/s/AKfycbzB-933ZJWT5AStO7BdhxZCCyvZeRHgNEja_pqXNZXmGl8rmcdzcSJ6dilOTAbwrdXf/exec';
  ...
  if (isLoggedIn) {
    ...
    if (username && password) {
      fetchDashboard(username, password); // Retrieve dashboard data without form submission
    }
  } else {
...
function fetchDashboard(username, password) {
  if (!username || !password) {
    showMessage("Missing username or password. Please log in again.", false);
  ...
  fetch(scriptURL, { method: 'POST', body: formData })
    ...
    .then(data => {
      if (data.result === "success") {
        displayDashboard(data.dashboardData); // Display dashboard with fetched data
      } else {
        // Clear localStorage and redirect to login on failure
        localStorage.removeItem("isLoggedIn");
        localStorage.removeItem("username");
        localStorage.removeItem("password");
        showMessage("Session expired or invalid credentials. Please log in again.", false);
        toggleForms('login-form');
      }
    })
    .catch(error => {
      console.error("Fetch error:", error); // Debugging
      showMessage('Error connecting to server. Please try again.', false);
    });
}
  ...
  if (validateForm(form, action)) {
    ...
    fetch(scriptURL, { method: 'POST', body: formData })
      ...
      .then(data => {
        ...
        if (data.result === "success") {
          if (action === 'login') {
             // First-time login: store credentials and display dashboard
            localStorage.setItem("isLoggedIn", "true");
            localStorage.setItem("username", form.querySelector('input[name="username"]').value);
            localStorage.setItem("password", form.querySelector('input[name="password"]').value);

            isLoggedIn = true; // Update global state
            displayDashboard(data.dashboardData); // Show dashboard with fetched data
