...
if (isLoggedIn) {
  // Load the dashboard data from local storage
  const dashboardData = JSON.parse(localStorage.getItem("dashboardData"));
  if (dashboardData) {
    displayDashboard(dashboardData);  // Display stored dashboard data
  }

  // Set a timeout to save the dashboard data and log the user out after 2 minutes
  setTimeout(() => {
    // Save dashboard data (assuming you want to save it again after the timeout)
    const currentDashboardData = getCurrentDashboardData(); // Replace with your function to get current data
    localStorage.setItem("dashboardData", JSON.stringify(currentDashboardData));

    // Log the user out by clearing the login status and dashboard data
    localStorage.setItem("isLoggedIn", "false");
    localStorage.removeItem("dashboardData");

    // Redirect to login page or adjust UI as needed
