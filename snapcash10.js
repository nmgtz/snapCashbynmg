...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (isLoggedIn && loginTimestamp && Date.now() - loginTimestamp <= twoDaysInMilliseconds) {
...
// Automatic login if the user succeeded to login within  48 hours
  ...
  fetch(scriptURL, { method: 'POST', body: formData })
    ...
    .then(data => {
      if (data.result === "success") {
        ...
        // Clear local storage if login fails
        logout();
    ...
    .catch(error => {
      ...
      showMessage("Unable to fetch user data. Please log in again.", false);
    });
}
      ...
      .then(data => {
        ...
        if (data.result === "success") {
          if (action === 'login') {
            ...
            localStorage.setItem("username", form.querySelector('input[name="username"]').value.trim());
            localStorage.setItem("password", form.querySelector('input[name="password"]').value.trim());
