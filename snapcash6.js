 const scriptURL = 'https://script.google.com/macros/s/AKfycbzB-933ZJWT5AStO7BdhxZCCyvZeRHgNEja_pqXNZXmGl8rmcdzcSJ6dilOTAbwrdXf/exec';
  ...
  // Helper function to check login expiration
  function isLoginValid() {
    const loginTimestamp = localStorage.getItem("loginTimestamp");
    if (!loginTimestamp) return false;
    
    const now = Date.now();
    return now - parseInt(loginTimestamp, 10) <= twoDaysInMs;
  }

  // Manage login state on page load
  function handleLoginState() {
    const isLoggedIn = localStorage.getItem("isLoggedIn") === "true";
    if (isLoggedIn && isLoginValid()) {
      displayDashboard(); // User is logged in and session is valid
    } else {
  ...
  // Handle password reset parameters
 ...
 function submitForm(action) {
  ...
  localStorage.setItem("loginTimestamp", Date.now().toString()); // Save login time
