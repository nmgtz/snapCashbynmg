 ...
 const scriptURL = 'https://script.google.com/macros/s/AKfycbzB-933ZJWT5AStO7BdhxZCCyvZeRHgNEja_pqXNZXmGl8rmcdzcSJ6dilOTAbwrdXf/exec';
  ...
  if (isLoggedIn) {
    ...
    if (username && password) {
      ...
      console.log("Attempting auto-login with credentials:", { username, password });

      // Trigger login using stored credentials
...
function submitForm(action, storedCredentials = null) {
  ...
  if (storedCredentials) {
    // Use credentials from storage for reload scenario
    formData = new FormData();
    formData.append("action", action);
    formData.append("username", storedCredentials.username);
    formData.append("password", storedCredentials.password);

    console.log("Using stored credentials for login:", storedCredentials); // Debug log
  } else {
